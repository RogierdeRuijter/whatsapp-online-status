<!-- Code stolen from https://github.com/yeukfei02/drawCanvas -->
<script lang="ts">
    import { onMount } from "svelte";
    import {
        addSquareCheckedStore,
        addCircleCheckedStore,
        addFlagCheckedStore,
    } from "../store";
    ...
    const draw = (context) => {
        ...
        context.strokeStyle = colorPicker.style.backgroundColor
    ...
    const setCanvasCircle = (context, circle) => {
        context.arc(
            circle.x,
            circle.y,
            circle.radius,
    ...
    const handleCanvas2Click = () => {
        console.log("handleCanvas2Click");
    ...
    const handleCanvas3MouseMove = (e) => {
    ...
    // TODO: figure out if we need to add the drawing instruction on the canvas itself would be cool
    const drawThings = () => {
        drawText();
        drawLine();
        ...
        const randomColor1 = `rgba(${redColor1}, ${greenColor1}, ${blueColor1}, ${opacity1.toFixed(
            1
        )})`;
        const randomColor2 = `rgba(${redColor2}, ${greenColor2}, ${blueColor2}, ${opacity2.toFixed(
            1
        )})`;
        const randomColor3 = `rgba(${redColor3}, ${greenColor3}, ${blueColor3}, ${opacity3.toFixed(
            1
        )})`;
    ...
    const drawRect = (x, y, width, height, fillStyle, strokeStyle) => {
        const context = canvas.getContext("2d");
        context.beginPath();
...
<div>
    <div class="drawCanvas">
        ...
        <!-- <div class="my-3">
            Press WASD to move circle. Hold mouse to draw. Double click to add
            item.
        </div> -->

        <!-- <input
            type="range"
            class="form-range slider my-2"
            min="1"
            on:change={handleSliderChange} /> -->

        <!-- TODO: find a better way to set the color of the color picker" -->
        <div
            class="colorPicker my-3"
            bind:this={colorPicker}
            style="display: none;" />

        <!-- <div class="my-2">
            <div>Choose pencil color:</div>
            <div class="colorPicker my-3" bind:this={colorPicker} />
            <canvas
                class="canvas2"
                bind:this={canvas2}
                on:mousemove={handleCanvas2MouseMove}
                on:click={handleCanvas2Click} />
        </div> -->

        <!-- <div class="my-2">
            <div>Choose background color:</div>
            <div class="bgColorPicker my-3" bind:this={bgColocPicker} />
            <canvas
                class="canvas3"
                bind:this={canvas3}
                on:mousemove={handleCanvas3MouseMove}
                on:click={handleCanvas3Click} />
        </div> -->
